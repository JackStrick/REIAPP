	...
	async (zpid, thunkAPI) => {
		try {
			...
			const message =
				error.response?.data?.message ||
				error.message ||
				"Error fetching property analytics";
			return thunkAPI.rejectWithValue(message);
		}
	}
);
...
// Create an asynchronous thunk for fetching user properties
export const isUserProperty = createAsyncThunk(
	"db/isUserProperty",
...
export const deleteUserProperty = createAsyncThunk(
	"db/deletedUserProperty",
	async ({ userId, propertyId }, thunkAPI) => {
		try {
			const response = await dbService.deleteUserProperty(
				userId,
				propertyId
			);
			return response;
...
export const createUserProperty = createAsyncThunk(
	...
	async ({ userId, propertyId }, thunkAPI) => {
		try {
			const response = await dbService.createUserProperty(
				userId,
				propertyId
			);
			return response;
